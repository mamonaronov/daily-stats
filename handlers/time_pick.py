...
from handlers.common import prompt_with_hint, require_writable, show_main
...
async def _finish(
    ...
    if purpose == "slp_bed":
        ...
        return
    if purpose == "slp_away":
        from handlers.sleep import complete_sleep_away

        await complete_sleep_away(event, state, repo, user, when)
        return
    item_id = None
    error = None
    if purpose == "cig":
...
def _onset_screen_text(data: dict) -> str:
    return data.get("onset_prompt") or prompt_with_hint("Когда заснули?", data)
...
async def _show_when_screen(cb: CallbackQuery, state: FSMContext, data: dict) -> None:
    ...
    if prefix == "slo":
        ...
        await state.set_state(None)
        await safe_edit(cb.message, _onset_screen_text(data), sleep_onset_kb(undo_kind, undo_id))
        return
    if prefix in {"slw", "slu", "slb", "sln", "sla"}:
...
async def when_ago_ask(cb: CallbackQuery, state: FSMContext, db_user: User | None) -> None:
    ...
    await safe_edit(
        cb.message,
        prompt_with_hint("Сколько времени назад это было?", await state.get_data()),
        ago_pick_kb(prefix),
    )
...
async def when_ago_number(cb: CallbackQuery, state: FSMContext, db_user: User | None) -> None:
    ...
    await safe_edit(
        cb.message,
        prompt_with_hint(AGO_MINUTES_PROMPT, await state.get_data()),
        back_kb(NAV_BACK),
    )
...
async def when_text_ask(cb: CallbackQuery, state: FSMContext, db_user: User | None) -> None:
    ...
    await safe_edit(
        cb.message,
        prompt_with_hint(WHEN_TEXT_PROMPT, await state.get_data()),
        back_kb(NAV_BACK),
    )
...
async def time_pick_back(cb: CallbackQuery, state: FSMContext, repo: Repo, db_user: User | None) -> None:
    ...
    if action == "ago_pick":
        ...
        await safe_edit(
            cb.message,
            prompt_with_hint("Сколько времени назад это было?", data),
            ago_pick_kb(prefix),
        )
        return
    ...
    if action == "date":
        ...
        await safe_edit(
            cb.message,
            prompt_with_hint("Выберите дату:", data),
            calendar_kb(day.year, day.month, back=NAV_BACK),
        )
        return
    day = date.fromisoformat(data.get("picked_date") or today.isoformat())
    await state.set_state(TimePickSG.hour)
...
async def pick_hour(cb: CallbackQuery, state: FSMContext, db_user: User | None) -> None:
    ...
    if token == "manual":
        ...
        await cb.answer()
        await safe_edit(cb.message, prompt_with_hint(MANUAL_TIME_PROMPT, data), back_kb(NAV_BACK))
        return
    ...
    await safe_edit(
        cb.message,
        prompt_with_hint(f"Час: {int(token):02d}\nВыберите минуты:", data),
        minutes_kb(),
    )
