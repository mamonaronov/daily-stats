...
from keyboards.main import cancel_kb, now_or_time, skip_comment_kb
...
async def act_type(cb: CallbackQuery, state: FSMContext, db_user: User | None) -> None:
    ...
    await cb.answer()
    await safe_edit(cb.message, "Длительность в минутах:", cancel_kb())
...
async def act_duration(message: Message, state: FSMContext, db_user: User | None) -> None:
    ...
    try:
        ...
        await message.answer("Введите число минут, например 35", reply_markup=cancel_kb())
        return
    await state.update_data(duration=duration)
    await state.set_state(ActivitySG.comment)
    await message.answer("Комментарий? Можно пропустить.", reply_markup=skip_comment_kb())
...
async def act_skip(cb: CallbackQuery, state: FSMContext, db_user: User | None) -> None:
    ...
    await cb.answer()
    await safe_edit(cb.message, "Когда была активность?", now_or_time("actt"))
...
async def act_comment(message: Message, state: FSMContext, db_user: User | None) -> None:
    ...
    await message.answer("Когда была активность?", reply_markup=now_or_time("actt"))
