...
from keyboards.main import cancel_kb, confirm_delete_kb, settings_kb, timezone_kb
...
async def settings_root(cb: CallbackQuery, db_user: User | None, repo: Repo) -> None:
...
async def set_tz(cb: CallbackQuery, state: FSMContext, db_user: User | None) -> None:
    ...
    await cb.answer()
    await safe_edit(cb.message, "Выберите часовой пояс:", timezone_kb())


@router.callback_query(F.data.startswith("tz:"), SettingsSG.timezone_custom)
async def set_tz_pick(
    ...
    if token == "custom":
        await cb.answer()
        await safe_edit(cb.message, "Введите IANA-имя, например Europe/Moscow", cancel_kb())
        return
...
async def set_tz_custom(
    ...
    if not is_valid_timezone(token):
        await message.answer("Не получилось распознать пояс.", reply_markup=cancel_kb())
        return
    await repo.set_timezone(user.telegram_id, token)
    user = await repo.get_user(user.telegram_id)
    assert user
...
async def set_sleep(cb: CallbackQuery, state: FSMContext, db_user: User | None) -> None:
    ...
    await cb.answer()
    await safe_edit(cb.message, "Введите обычное время сна ЧЧ:ММ", cancel_kb())
...
async def save_sleep(
    ...
    try:
        ...
        await message.answer("Пример: 23:30", reply_markup=cancel_kb())
        return
